...
@section('content')
    ...
    >
        @if($errors->any())
            <div class="notification rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700 dark:border-red-900/50 dark:bg-red-900/30 dark:text-red-300">
            ...
            <section class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                ...
                <form method="GET" action="{{ route('admin.maintenance-gallery.index') }}" class="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-[minmax(0,1fr)_10rem_10rem_auto]">
                    ...
                    <x-search-field
                        ...
                        name="q"
                        :value="$search"
                        placeholder="Search property, serial, type, or caption"
                        ariaLabel="Search maintenance gallery"
                        inputClass="min-h-11 w-full rounded-lg border border-gray-300 bg-white px-3 pr-20 text-sm text-gray-900 placeholder-gray-400 focus:border-blue-500 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400"
                    />
