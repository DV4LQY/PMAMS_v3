...
<div class="mx-auto max-w-5xl space-y-5">
    <div class="flex flex-wrap items-center justify-between gap-3"><div><h1 class="text-xl font-semibold">Checklist report preview</h1><p class="text-sm text-gray-500">{{ $record->device->property_number }} · {{ $record->maintenance_date?->format('M d, Y') }}</p></div><a data-no-spa="true" target="_blank" href="{{ route('admin.reports.checkedEquipment.pdf', $record) }}" class="inline-flex items-center justify-center rounded-lg bg-blue-600 px-4 py-2 text-center text-sm font-semibold leading-5 text-white">Open PDF</a></div>
